...
# Вынесено в krepost.utils.frontmatter — единый контракт для ingestion и trust_bridge
_parse_existing_frontmatter = parse_existing_frontmatter
...
class InboxWatcher:
    ...
    def _enqueue(self, path_str: str) -> None:
        ...
        if self._loop is not None and not self._loop.is_closed():
            try:
                self._loop.call_soon_threadsafe(self._schedule_in_loop, path_str)
            except RuntimeError:
                pass  # loop закрыт при shutdown

    def _schedule_in_loop(self, path_str: str) -> None:
        now = time.time()
        self._pending[path_str] = now
        asyncio.create_task(self._delayed_ingest(path_str, now))

    async def _delayed_ingest(self, path_str: str, scheduled_at: float) -> None:
        await asyncio.sleep(self.debounce_sec)
        if self._pending.get(path_str, 0) != scheduled_at:
            return
        self._pending.pop(path_str, None)
        try:
            await self.ingestion.ingest_file(Path(path_str))
        except Exception:
            logger.exception(f"Watcher ingest failed for {path_str}")
...
# ═══════════════════════════════════════════════════════════════════════════
# VaultWatcher — invalidate-only; работа с _pending в loop (P1-2)
# ═══════════════════════════════════════════════════════════════════════════

class VaultWatcher:
    def __init__(self, vault_dir: Path, on_note_changed: Callable[[str], None],
    ...
    def start(self, loop: asyncio.AbstractEventLoop) -> None:
        ...
        def is_md(p: str) -> bool:
            # фильтр: .md, не .md.tmp, внутри ingested
            return p.endswith(".md") and not p.endswith(".tmp")

        class Handler(FileSystemEventHandler):
            def on_modified(self, event):
                if not event.is_directory and is_md(event.src_path):
                    watcher._enqueue_schedule(event.src_path)

            def on_deleted(self, event):
                if not event.is_directory and is_md(event.src_path):
                    watcher._enqueue_now(event.src_path)

            def on_moved(self, event):
                if not event.is_directory:
                    if is_md(event.src_path):
                        watcher._enqueue_now(event.src_path)
                    if is_md(event.dest_path):
                        watcher._enqueue_schedule(event.dest_path)

        self._observer = Observer()
        self._observer.schedule(Handler(), str(self.vault_dir), recursive=True)
        self._observer.start()
        logger.info(f"VaultWatcher started on {self.vault_dir}")

    def _enqueue_schedule(self, path_str: str) -> None:
        if self._loop is not None and not self._loop.is_closed():
            try:
                self._loop.call_soon_threadsafe(self._schedule_in_loop, path_str)
            except RuntimeError:
    ...
    def _enqueue_now(self, path_str: str) -> None:
        if self._loop is not None and not self._loop.is_closed():
            try:
                self._loop.call_soon_threadsafe(self._invalidate_now, path_str)
            except RuntimeError:
                pass
    ...
    def _schedule_in_loop(self, path_str: str) -> None:
        now = time.time()
        self._pending[path_str] = now
        asyncio.create_task(self._delayed_invalidate(path_str, now))

    async def _delayed_invalidate(self, path_str: str, scheduled_at: float) -> None:
        await asyncio.sleep(self.debounce_sec)
        if self._pending.get(path_str, 0) != scheduled_at:
            return
        self._pending.pop(path_str, None)
        self._invalidate_now(path_str)

    def _invalidate_now(self, path_str: str) -> None:
        # P0-3: поддержка sync и async callback
        cb = self.on_note_changed
        try:
            if asyncio.iscoroutinefunction(cb):
                if self._loop is not None and not self._loop.is_closed():
                    asyncio.run_coroutine_threadsafe(cb(path_str), self._loop)
            else:
                cb(path_str)
        except Exception:
            logger.exception(f"on_note_changed callback failed for {path_str}")
...
async def _smoke():
    ...
    def tg(e: IngestEvent):
        if e.level in (IngestEventLevel.YELLOW, IngestEventLevel.RED):
            print(f"  TG: [{e.level.value}] {e.message}")

    def cache(n: str):
        print(f"  cache.invalidate_by_note({n})")

    ing = DocumentIngestion(base_dir=Path("inbox"), vault_dir=Path("vault"),
                            on_event=tg, on_note_changed=cache)
    Path("inbox").mkdir(exist_ok=True)
    Path("inbox/test.md").write_text("# Тест\nЭто тестовый документ.", encoding="utf-8")
    r = await ing.ingest_file(Path("inbox/test.md"))
    print(f"Single: {r.status} | {r.chars} chars | quarantine={r.quarantine}")
    rep = await ing.ingest_directory(Path("inbox"))
    print(f"Batch: total={rep.total} success={rep.success} skipped={rep.skipped} failed={rep.failed}")
