...
КОНТРАКТ ХЕШИРОВАНИЯ (зафиксирован в security v1.3):
  ...
  ingestion (parse_existing_frontmatter).

ПОДКЛЮЧЕНИЕ (в app.py):
    from krepost.security import TrustRegistry
    from krepost.ingestion.document_ingestion import VaultWatcher
...
# единый парсер frontmatter — общий контракт хеширования с ingestion
...
class TrustBridge:
    ...
    def _read_body(self, path: Path) -> Optional[str]:
        ...
        try:
            ...
            return None
        _existing, body = parse_existing_frontmatter(content)
        return body
